...
func (c *Cursor) Move(x int, y int) *Cursor {
	...
	if x < 0 {
		sequence.WriteString("\x1b[" + strconv.Itoa(int(math.Abs(float64(x)))) + "D") // Left
	} else if x > 0 {
		sequence.WriteString("\x1b[" + strconv.Itoa(x) + "C") // Right
	}

	if y < 0 {
		sequence.WriteString("\x1b[" + strconv.Itoa(int(math.Abs(float64(y)))) + "A") // Up
	} else if y > 0 {
		sequence.WriteString("\x1b[" + strconv.Itoa(y) + "B") // Down
	}

	c.Output.Write(sequence.String(), false, 0)
	return c
}
